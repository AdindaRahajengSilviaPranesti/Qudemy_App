...
import { NgbdBuySellSortableHeader, buysellSortEvent } from './buy-sell-sortable.directive';
...
export class BuySellComponent {
  ...
  // Table data
  buysellList$!: Observable<MarketModel[]>;
  total$: Observable<number>;
  @ViewChildren(NgbdBuySellSortableHeader) headers!: QueryList<NgbdBuySellSortableHeader>;

  constructor(public service: BuySellService) {
    this.buysellList$ = service.countries$;
...
*
*/
  onSort({ column, direction }: buysellSortEvent) {
    // resetting other headers
    this.headers.forEach((header) => {
      if (header.buysellsortable !== column) {
        header.direction = '';
      }
    });
